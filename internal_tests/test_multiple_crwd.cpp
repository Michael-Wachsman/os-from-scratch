...
// let's keep creating a deleting same dirs
void deletr() {
    for (int i = 0; i < 40; i += 4) {
        printf("running a delete %d\n", i);
        int status
          = fs_delete("user1", ("/second/" + std::to_string(i) + ".txt").c_str());
        printf("completed a delete %d\n", i);
    }
}

void creatr() {
    for (int i = 0; i < 40; i += 4) {
        printf("running a create %d\n", i);
        int status = fs_create(
          "user1", ("/second/" + std::to_string(i) + ".txt").c_str(), 'f');

        printf("completed a create %d\n", i);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::thread t4(deletr);
    std::thread t5(deletr);
    std::thread t3(creatr);
    std::thread t6(creatr);
    t0.join();
    t1.join();
    t2.join();
    t3.join();
